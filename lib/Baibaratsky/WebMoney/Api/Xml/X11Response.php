 ...
 */
class X11Response extends ApiResponse
{
    /** @var string /@retdesc */
    ...
    /** @var X11ResponsePassport[] certinfo/attestat */
    protected $_passports = array();

    /** @var X11ResponseWmid[] certinfo/wmids */
    protected $_wmids = array();

    /** @var X11ResponseUserInfo[] certinfo/userinfo/value */
    protected $_userInfo = array();

    /** @var X11ResponseCheckLock[] certinfo/userinfo/check-lock */
    protected $_checkLock = array();

    /** @var X11ResponseWebList[] certinfo/userinfo/weblist */
    protected $_webList = array();

    /** @var X11ResponseExtendedData[] certinfo/userinfo/extendeddata */
    ...
    public function __construct($response)
    {
        ...
        if ($certInfo->attestat !== null) {
            foreach ($this->_rowAttributesXmlToArray($certInfo->attestat) as $passport) {
                $this->_passports[] = new X11ResponsePassport($passport);
            }
        }

        if ($certInfo->wmids !== null) {
            foreach ($this->_rowAttributesXmlToArray($certInfo->wmids) as $wmid) {
                $this->_wmids[] = new X11ResponseWmid($wmid);
            }
        }
        ...
        if ($certInfo->userinfo !== null) {
            ...
            if ($certInfo->userinfo->extendeddata !== null) {
                foreach ($this->_rowAttributesXmlToArray($certInfo->userinfo->extendeddata) as $extendedData) {
                    $this->_extendedData[] = new X11ResponseExtendedData($extendedData);
                }
            }
        }
    }
    ...
    /**
     * @return X11ResponseExtendedData[]
     */
