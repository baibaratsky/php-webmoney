...
use Baibaratsky\WebMoney\Exception\SignerException;
...
class RequestSigner
{
    ...
    {
        if (empty($wmid)) {
            throw new SignerException('WMID not provided.');
        }

        if (!file_exists($keyFileName)) {
            throw new SignerException('Key file not found: ' . $keyFileName);
        }
        ...
        if (!$this->_verifyHash($keyData)) {
            throw new SignerException('Hash check failed. Key file seems corrupted.');
        }
